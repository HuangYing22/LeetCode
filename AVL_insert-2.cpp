    ...
    while(!stPath.empty()) {
        ...
        if(node->leftHeight - node->rightHeight == 2) {
            if(move->leftHeight - move->rightHeight == 1) {
                ...
                cwRotate(root, node, move, stPath);
                ...
                moveRight->left = move;
                //CW
                cwRotate(root, node, moveRight, stPath);
            }
        } 
        if(node->leftHeight - node->rightHeight == -2) {
            if(move->leftHeight - move->rightHeight == -1) {
                //R-R Single Rotate: Counter-Clock-wise rotate
                ccwRotate(root, node, move, stPath);
            } else if(move->leftHeight - move->rightHeight == 1){
                ...
                moveLeft->right = move;
                //CCW
                ccwRotate(root, node, moveLeft, stPath);
            }
        }
