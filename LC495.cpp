...
public:
    int findPoisonedDuration(vector<int>& timeSeries, int duration) {
        ...
        int max = timeSeries[n - 1] + duration;
        vector<bool> attacks(max, false);
        
        for(int i = 0; i < n; i ++) {
            for(int j = 0; j < duration; j ++) {
                if(!attacks[timeSeries[i] + j]) {
                    attacks[timeSeries[i] + j] = true;
                }
            }
        }
...
public:
    int findPoisonedDuration(vector<int>& timeSeries, int duration) {
        ...
        for(int i = 0; i < n; i ++) {
            ...
            } else if(timeSeries[i] + duration > max) {
                res += timeSeries[i] + duration - max;
                max = timeSeries[i] + duration;
            }
        }
        return res;
